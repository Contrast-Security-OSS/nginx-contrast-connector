 ...
 */
static const char DEFAULT_NAME[] = "__cs_default__";
		...
		void * child)
{
	...
	dd("APP NAME = %s %s", conf->app_name.data, prev->app_name.data); 
...
static ngx_int_t ngx_http_contrast_connector_module_init(ngx_conf_t * cf)
{
	ngx_http_core_main_conf_t *main_conf = ngx_http_conf_get_module_main_conf(cf, 
